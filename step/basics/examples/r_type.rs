...
pub trait Write {
    ...
    // 2023/8/31 c++中的别名是 using a = b； a是b类型的别名。
...
// 自己的理解是: rust 是函数式编程,函数式编程都是有返回值的,
// never 类型作为无函数返回的类型,那么统一了函数式编程.
fn type3() {
    let guess = "";
    loop {
...
// - 使用动态大小类型时总会把它的值放在某种指针后边 , 比如 4 中的 &str
// trait 也是动态大小类型
...
// 默认情况下,泛型函数只能用于编译时已经知道大小的类型,可以通过特殊语法接触这一限制.
